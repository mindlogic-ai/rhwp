...
### 정정
- HWPX 저장에서 선(line/connectLine)의 좌표를 `hp:startPt`/`hp:endPt` 로 쓰던 것을
  규격대로 `hc:startPt`/`hc:endPt` 로 정정했다. 한컴은 이 요소를 core(hc)
  네임스페이스로 쓴다. 접두사가 틀리면 그 문서를 한글이 열지 못하고("파일을 읽거나
  저장하는 데 오류가 있습니다"), 한컴 Docs Converter 는 필터 데몬이 응답을 멈춘
  채 code 0220 을 반환한다. 우리 파서는 로컬명만 보고 읽어서 자기 왕복은 통과했기
  때문에 드러나지 않았다 — 실제 재현은 바탕쪽에 묶음 개체(선 포함)가 있는 문서.
