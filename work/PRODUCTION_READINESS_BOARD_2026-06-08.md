...
| Photo/table page-count drift | `photo_form24` | probe/rejected-candidates | Fresh debug board shows Hancom `89` / RHWP `86`. Heading drift starts before the contact pages: `□ 주요 편성·확대 과제` is aligned at page 10, but `□ (범부처 협업 강화)` is RHWP page 11 vs Hancom page 12. Terminal-contact candidates reached `88/89` or `89/89` but inserted pages at the wrong visual location; a late heading/table candidate reached `87/89` or `88/89` but pushed the page-10 heading one page late. All were rejected and reverted. | dominant remaining issue is upstream under-height/packing around dense text plus TAC tables, especially the table/section transition around `pi=168`; page count alone is not a safe acceptance signal | open semantic blocker; needs a discriminator that preserves heading map, not just total count | open `/tmp/diff/_review_photo_form24_first_drift_2026-06-08/index.html`, `/tmp/diff/_review_photo_form24_terminal_contact_candidate_safe_2026-06-08/index.html`, and compare heading maps from `harness/heading_drift_scan.py photo_form24 --page-min 1 --page-max 24` |
