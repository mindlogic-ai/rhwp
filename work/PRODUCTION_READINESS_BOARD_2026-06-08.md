...
- Real portrait corpus page-count status: 20/21 clean. The only current
  real page-count gap is `photo_form24`, Hancom `89` / RHWP `87`.
...
| Photo/table page-count drift | `photo_form24` | accepted-local/probe-open | Tail-guard combo keeps the one-line tail before the explicit title-table page break and defers the following late heading/spacer/CELL TAC table. The next accepted local fix preserves the Hancom blank page before the `유학생 특화형` explicit top-reset paragraph and applies a narrow near-top heading reset for the following `성과관리` subsection. The latest local fix moves the small contact/signoff CELL TAC table after the large performance table spacer tail onto its own page before the next explicit section title. Focus windows now align `󰊲 대학생 현장실습 안전관리 지원`, `󰊳 대학생 현장실습 성과관리`, the contact page, and `【 대학창업 활성화 】`. Total page count improves to Hancom `89` / RHWP `87`. Previous terminal-contact candidates reached `88/89` or `89/89` but inserted pages at the wrong visual location and remain rejected. | remaining issue starts later in the `대학창업` section; page count alone is not a safe acceptance signal | open semantic blocker, first three drift windows locally improved and guarded | open `/tmp/diff/_review_photo_form24_pages_27_31_contact_table_defer_candidate_2026-06-08/index.html`; compare `python3 harness/page_heading_map.py photo_form24 --dump /tmp/diff/photo_form24/dump_pages_contact_table_defer_candidate_2026-06-08.txt --needle '󰊳 대학생 현장실습 성과관리' --needle '【 대학창업 활성화 】' --needle '□ 대학 중심 창업교육 기반 구축이 지역 혁신성장의 핵심 과제로 부상'` |
...
3. For render fidelity, continue the next unresolved non-landscape mismatch:
   `photo_form24` page-count drift (`89/87`) as the primary under-height
